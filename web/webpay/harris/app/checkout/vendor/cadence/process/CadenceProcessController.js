app.controller("CadenceProcessController",
	...
	function($scope,$rootScope,$log,$location,
            $timeout,$http,
            paymentFactory,errorManager) {
	$log.debug("Cadence Process Controller: Loaded");




	$log.debug("Cadence Process Controller: Payment data loading");
	$scope.payment = paymentFactory.get();
	if ( ! $scope.payment ) {
		$log.error("Cadence Process Controller: Failed to find payment data");
		errorManager.raiseDataError();
		return;
	}
	$scope.payment = $scope.payment.payment || $scope.payment;
	$log.debug("Cadence Process Controller: Payment data loaded");


    $scope.config = $rootScope.config;
	if ( ! $scope.config ) {
		$log.error("Cadence Process Controller: Failed to locate configuration");
		errorManager.raiseDataError();
		return;
	}
    ...
    if ( $scope.payment.status != $scope.status.VERIFIED ) {
        if ( $scope.payment.status == $scope.status.SUCCESS ) {
            $log.debug("Cadence Process Controller: Payment successful, redirecting");
            $location.path($scope.pages.SUCCESS);
            return;
        }
        if ( $scope.payment.status == $scope.status.FAILURE ) {
            $log.debug("Cadence Process Controller: Payment rejected, redirecting");
            $location.path($scope.pages.FAILURE);
            return;
        }

        // No other states should be directed to this page
        $log.error("Cadence Process Controller: Unexpected status: " + $scope.payment.status);
        errorManager.raiseStateError();
        return;
    }
    $scope.payment.status = $scope.status.PROCESSING;
    $log.debug("Cadence Process Controller: Processing payment");
    ...
    $scope.setPaymentParameters = function () {
        $log.debug("Cadence Process Controller: Setting payment parameters");
        var results = [];
    ...
    $scope.transactionWaitLimit = function(payment,pages,status) {
        ...
        $log.debug("Cadence Process Controller: Process time limit exceeded, checking status");
        if ( payment.status == status.PROCESSING ) {
            $log.debug("Cadence Process Controller: Process time limit exceeded, payment is still processing. Redirecting to " + pages.ERROR);
        ...
        $log.debug("Cadence Process Controller: Process time limit exceeded, payment status is not processing " + payment.status + ". It's not processing so we're abandoning this check");
    }
    ...
    $scope.savePaymentInformation = function () {
        $log.debug("Cadence Process Controller: Recording payment information");
        ...
        $http({
            method: "POST",
            url: $rootScope.config.urls.vendorPayment.Cadence,
            ...
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                ...
                $log.error("Cadence Process Controller: Server returned an error status - ("
                                + response.status + ")");
    ...
    $scope.processorFinished = function (response) {
            $log.debug("Cadence Process Controller: Analyzing payment processing response");
            ...
            $log.debug("Cadence Process Controller: TID " + $scope.payment.reference.tid);


            // Check processing status
            $log.debug("Cadence Process Controller: Verifying processing response status");
            if ( ! response.data.status || response.data.status != "OK" ) {
                $log.debug("Cadence Process Controller: Processing error response. Processing status not 'OK' as expected: " + response.data.status);
            ...
            $log.debug("Cadence Process Controller: Verifying processing response format");
            if ( ! response.data.reference ) {
                $log.debug("Cadence Process Controller: Invalid response, no 'reference' found in response");
                $scope.payment.status = $scope.status.FAILURE;
            ...
            $log.debug("Cadence Process Controller: Verifying payment status");
            if ( response.data.reference.status == "success" ) {
                $log.debug("Cadence Process Controller: Payment was successful, reference: " + response.data.reference.ref);
            ...
            // $scope.payment.reference
            $log.debug("Cadence Process Controller: Payment was NOT successful, status: " + response.data.reference.status);
            $scope.payment.status = $scope.status.FAILURE;
            $scope.payment.reference.auth = (response.data.reference.auth 
                                                            ? response.data.reference.auth 
                                                            : $scope.payment.reference.tid
            ...
            if ( response.data.reference.status == "failure" ) {
                $log.debug("Cadence Process Controller: Payment failed, reference: " + response.data.reference);
                $location.path($scope.pages.FAILURE);
                return;
            }

            // We have a systemic error, the response received is not one we've programmed to handle
            $log.debug("Cadence Process Controller: Unknown payment status: " + response.data.reference.status);
    ...
    $scope.clearPaymentCart = function () {
        if ( $rootScope.config.urls.clearCart == null ) {
            $log.debug("Cadence Process Controller: Unable to clear cart of accounts");
            return;
        }

        $log.debug("Cadence Process Controller: Submitting to clear cart of accounts");
        ...
        $http({
                ...
                $log.error("Cadence Process Controller: Cart clear request returned an error status - ("
                                + response.status + ")");
                $log.error("Cadence Process Controller: Response ignored, no action to be taken");
                $log.debug(JSON.stringify(response.data));
                return;
            });
    };
